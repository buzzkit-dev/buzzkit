...
| **E9** | **Workflows III — measurement and control** | Decided 2026-08-30, after E6's review: `split` (weighted arms with a stable per-subscriber hash, rendered as a branch) and `goal: { event, within }` (runs record `$run.converted`, conversion per version and arm on the run pages); `repeat: { steps, until, every?, max }` for daily reminders until an event; `waitFor` on several events (`steps.x.event`), `where` on `resetOn` entries and `endOn` events that end the wait; frequency caps and quiet hours as tenant send policy (`workflows.quietHours`, `workflows.dailyCap`; quiet hours defer to the next allowed local time, caps skip and record why; applies to scheduled messages too, so it lives with messages); `subscriber.channels` / `subscriber.topics` in scope; rich payloads (`actions`, `image`, `sound`, `badge`, `threadId`, `collapseId`). Grammar settled key by key on the capabilities page first, then built. Later still: aggregates over event fields, Tinybird deep history, segment enter/leave triggers, collect-and-digest, per-subscriber locale. |
