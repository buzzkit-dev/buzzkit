...
- **`deliver: "local"`** ships with the iOS SDK phase (E8), designed in from E5 (the `send` step carries `deliver`).
...
One JSON document per version, validated by `buzzkit/workflows` (TypeBox schema + a lint that checks paths, step names and references, exactly like `buzzkit/expressions`), so the SDK, the API and the dashboard share one definition and the platform dogfoods the package.
...
```jsonc
{
  ...
  "steps": [
    ...
    { "name": "outcome", "branch": { "if": { "ref": "steps.cancel.matched", "eq": true },
        "then": [{ "name": "sorry", "send": { "title": "Your trial is cancelled", "body": "Alerts continue until the end." } }],
...
| DELETE | `/v1/workflows/:slug` | Soft delete, removed from KV, live runs terminated (`$run.cancelled`, reason `workflow_deleted`) |
...
Audit + webhook events: `workflow.created / updated / published / paused / deleted`. Stream events (`$run.started / step / completed / cancelled / failed`) are what the run pages and webhooks on the data plane read. Errors: `invalid_spec` (with the lint's path), `slug_taken`, `slug_reserved`, `workflow_not_active`, `event_reserved` (a trigger on `$run.*`).

### Dashboard (Messaging → Workflows)

- **List**: name with the trigger summary underneath ("on `trial.started` from server"), `WorkflowStatusBadge` (Draft grey / Active green / Paused amber), the live counts as three quiet numbers, Updated. **Create workflow** opens the editor.
...
1. `buzzkit/workflows` (schema, lint, duration parsing, unit tests).
...
| **E6** | **Workflows II — data and time** | `fetch` (signed, retried, `onError`), templates, `set`, `opened(step)` / `since: trigger` / `count` conditions, local-time `waitUntil`, **schedule triggers** (actor `schedule()`), `afterBackground`, `skipIfSentWithin`, `POST …/test` (dry run), **schedule triggers over a segment** (the recurring send, taking over the cron parser E4 carried) | The full trial workflow (both fetches) and the streak workflow deliver the right pushes to a phone in the right local hour; the dry run reproduces the trace without sending |
| **E7** | **Sources — inbound webhooks** | A **source** is an inbound webhook endpoint of a tenant that turns a third party's webhooks into stream events: `POST /v1/sources` with a provider (`stripe`, `superwall`, `generic`) → a unique ingest URL (`/v1/sources/:id/ingest`) and secret; the provider's signature scheme verified per provider (Stripe `Stripe-Signature`, Superwall's HMAC, a shared secret header for generic); a **mapping** from the payload to an event: which subscriber (`externalId` from a path like `data.object.metadata.userId` or `customer`), the event name (the provider's `type`, optionally renamed, `subscription.started` for `customer.subscription.created`), the `data` to keep (picked paths), and a `where` on the payload so only some types pass; deduplicated by the provider's event id; unmatched payloads recorded but dropped. Provider presets ship the verification and a default mapping. The events land on the stream like any tracked event (`source: 'webhook'`), so segments and workflow triggers see `subscription.started` from Stripe the same as from your backend. **Dashboard**: Developers → Sources (list, create with a preset, edit the mapping with a sample payload and a live preview of the event it becomes, deliveries log with the raw payload and what happened to it) | A Stripe `customer.subscription.created` for a known customer starts the trial workflow's `subscription.started` cancel rule with no code on the customer's side; a replayed webhook is a duplicate; a bad signature is rejected and logged |
...
| **E9** | **Code** | `buzzkit` builders (`defineWorkflow` / `defineSegment` → spec) and `buzz.workflows.upsert` / `buzz.segments.upsert` in the server SDK: definitions live in the customer's codebase and are applied from code (a deploy step, a script), diffed against the deployment; no CLI | The trial and streak workflows defined in a demo repo, applied from code, phones buzz; a changed body shows a real diff and a new version |
