...
| **E8** | **Code** | `buzzkit` builders (`defineWorkflow` / `defineSegment` / `defineCampaign` → spec) and `buzz.workflows.upsert` / `buzz.segments.upsert` in the server SDK: definitions live in the customer's codebase and are applied from code (a deploy step, a script), diffed against the deployment; no CLI | The trial and streak workflows defined in a demo repo, applied from code, phones buzz; a changed body shows a real diff and a new version |
