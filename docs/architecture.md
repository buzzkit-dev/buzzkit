...
- Secrets: `BETTER_AUTH_SECRET`; `CREDENTIAL_MASTER_KEY` (exactly 32 random bytes, base64 — the vault refuses any other length); optional `CREDENTIAL_MASTER_KEYS` (JSON `{ "1": …, "2": … }`) + `CREDENTIAL_MASTER_KEY_VERSION` for rotation — new credentials seal under the current version, older versions stay readable, and the key version is bound into each credential's AAD; `SQIDS_ALPHABET` (the committed value is a dev default — override as a secret); `AXIOM_*` / `OTEL_EXPORTER_OTLP_ENDPOINT` as needed.
