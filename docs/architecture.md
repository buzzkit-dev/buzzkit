...
queue/     queue consumers (deliveries, events) and the reconciliation cron
libs/      infrastructure plugins: response envelope, errors, sqids, logger, database, tinybird (client, ingest, JWT), actor (stub lookup)
```
...
- **Fan-out** pages use a partial composite index `(tenant_id, channel, id) where enabled and active and not deleted` — the exact shape of the page query; `to`-targeting carries the tenant predicate on both `subscription` and `subscriber`, so it hits the `(tenant_id, external_id)` unique index instead of walking the tenant's subscriptions. A `(message_id, status, id)` index makes the completion probe, status-filtered delivery listing, and the completion recount index-only.
- **Delivery processing is batched in three phases** so the database is idle while providers are called: one projected `SELECT … WHERE id = ANY` for the whole batch and one `UPDATE … FROM (VALUES …)` lease claim; provider sends at `SEND_CONCURRENCY` (4) with credentials decrypted once per tenant×provider×environment and provider tokens memoized per batch above `PROVIDER_CACHE`; then one transaction with a multi-row attempt insert (`ON CONFLICT DO NOTHING` = the exactly-once guard), one `UPDATE … FROM (VALUES …)` for every delivery, one for every invalidated subscription, one batched `subscription.invalidated` insert, counters once per message, and a single `sendBatch` for every retry. ~8 statements per 100 deliveries instead of ~700. The queue's Postgres client holds 2 connections and the consumer is capped at `max_concurrency: 40`, so 40 × 2 stays inside Hyperdrive's pool and 2 + 4 provider fetches stays inside the 6-connection Worker limit.
...
- **Reconciliation** runs on partial indexes only (`next_attempt_at` for due retries, the `coalesce(lease_expires_at, created_at)` expression index for lost enqueues and crashed leases, `expires_at` for expiries, `updated_at` for stalled or never-started fan-outs and unfinalized messages) — sweeps never scan, and each bucket drains up to 10 × 1,000 rows per tick so a backlog heals in minutes, not days. A message whose very first enqueue was lost (still `queued`) is re-driven like a stalled one.
...
- **One Worker, three services.** `instrument()` (`@microlabs/otel-cf-workers`) wraps the exported handler and resolves its config per invocation from the trigger, so traces are reported as **`buzzkit-api`** (fetch), **`buzzkit-queue`** (queue batches), **`buzzkit-scheduler`** (cron) and **`buzzkit-actor`** (the subscriber Durable Object, instrumented with `instrumentDO`; its `actor.ingest` span is a child of the API request's `events.ingest` span through the `traceparent` carried on the RPC, and `actor.flush` is its own trace) — separate services in Axiom without separate deployables. The root span per invocation is created by the library (`fetchHandler`, `queueHandler buzzkit-deliveries`, `scheduledHandler`), which also auto-instruments outbound `fetch` (every APNs/FCM/Resend call), KV, Queue sends, and the cache API.
...
Non-secret config in `wrangler.jsonc` `vars` (override per environment); secrets via `.dev.vars` locally and `wrangler secret` deployed.

**Deployment checklist (security-relevant):**

- Secrets: `BETTER_AUTH_SECRET`; `CREDENTIAL_MASTER_KEY_V1…` (exactly 32 random bytes each, base64 — the vault refuses any other length; rotate by adding the next version, the scheduler re-wraps, then drop the old one); `SQIDS_ALPHABET` (generated per deployment, never changed afterwards); `AXIOM_*` / `OTEL_EXPORTER_OTLP_ENDPOINT` as needed.
- `DASHBOARD_URL` is load-bearing: it is the only CORS origin (with credentials) and a BetterAuth trusted origin. `ENVIRONMENT` must be `production` (or anything but `development`) deployed — only `development` disables BetterAuth's CSRF/origin check, and only `development` returns raw messages for unexpected errors (elsewhere they are logged and masked).
- Rate limiting: BetterAuth's built-in limiter is in-memory per isolate and therefore ineffective on Workers; put Cloudflare WAF rate-limit rules on `/v1/auth/sign-in/*`, `/v1/auth/sign-up/*`, and `/v1/client/*` at deploy time (an app-level KV/DO limiter is Phase 10).
- Email verification on sign-up is not yet enforced (lands with the dashboard auth pages); until then an invite binds to an email that anyone could claim at sign-up, which is why invites are session-only.
