...
- **Reconciliation** runs on partial indexes only (`next_attempt_at` for due/stale deliveries, `expires_at` for expiries, `updated_at` for stalled fan-outs) — sweeps never scan.
