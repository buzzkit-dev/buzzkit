...
- **Reconciliation**: every five minutes the sweep re-enqueues public audit rows of the last hour that have no `webhook_event` and belong to a workspace with a live endpoint, and re-enqueues deliveries whose retry never came (queue hiccups). A lost enqueue heals in minutes.
