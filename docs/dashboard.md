...
3. Route **loaders** read the cookie (`lib/session.server.ts`: `requireSession`, `readSessionToken`) and forward its value, BetterAuth's signed session token, as the bearer header on server-side `/v1/*` calls via `lib/api.server.ts`. The browser never sees the token: the cookie is httpOnly and nothing is stored client-side.
4. `/logout` is a route action: it calls `/v1/auth/sign-out` with the bearer token and expires the cookie (on the same derived domain). Any API 401 does the same (`signedOutRedirect`).
...
`/login` and `/signup` share one pathless layout (`routes/auth/layout.tsx` → `components/auth/shell.tsx`) that renders **one persistent form** (`components/auth/form.tsx`, `AuthForm`): the routes only contribute `handle.auth` (mode, title, description, footer link) and loader data (`apiUrl`, providers, redirect, error), and render nothing themselves. Switching between the two keeps Email, Password and the submit button exactly where they are; the Name field is the only thing that moves, rising out from behind Email (its grid row unfolding from `0fr` to `1fr` while the field inside keeps its natural height, translate up from 24px, scale from 0.92, fade in, 0.4s bounce-0 spring) and folding back the same way; the "Must be at least 8 characters" hint under Password does the mirror image, unfolding downward from behind the input. The fields below Name sit on their own `bg-card` layer so Name really comes from behind them. `/new` and `/invite/:token` use the static `AuthLayout` card.
