...
- `GET /v1/messages` — newest-first, keyset-paginated with `total`. Filters combine with AND: `q` (case-insensitive match on title, body, topic and the target ids), `status` (`queued` / `processing` / `completed`), `channel`, `topic` (slug), `from` / `to` (ISO timestamps on `createdAt`).
