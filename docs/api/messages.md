...
- **Content**: at least one of `title`, `body`, `data`. Optional `subtitle`, `badge`, `sound`, `imageUrl`, `collapseId`, `priority` (`high` default | `normal`), `threadId` (groups notifications), `interruptionLevel` (`passive` | `active` | `timeSensitive` | `critical`), `relevanceScore` (0 to 1), `category`, `targetContentId`, `actions` (up to four buttons: `id`, `title`, `destructive`, `foreground`, `input` + `placeholder`; the iOS SDK registers them and the opened receipt carries the tapped id and typed text), `deepLink` (opened by the app on tap), `action` (`{ name, data }`, runs a handler the app registered), `policy: "ignore"` (bypasses the tenant send policy — the security-alert class), and raw escape hatches `apns.payload` / `fcm.android` / `fcm.payload`. `apns.environment` picks sandbox vs production credentials (default production; falls back to whichever exists).
...
`PATCH /v1/tenants/:slug { "settings": { "sendPolicy": { "quietHours": { "from": "22:00", "to": "08:00", "timezone": "subscriber" }, "dailyCap": 3 } } }` — off by default, each half independent, `null` clears one. The policy applies to every visible push on the tenant: workflows, scheduled messages, and direct API sends alike. Quiet hours defer the delivery to the next allowed local time (`timezone: "subscriber"` follows each person's `$timezone`; a fixed IANA zone reads one clock; the delivery shows the deferred `nextAttemptAt`). The daily cap counts sent deliveries per subscriber per local day; a send past it fails as `capped` in the ledger. A payload with `"policy": "ignore"` passes through, and silent pushes (local-delivery carriers included) are always exempt — the plan's fire time respects quiet hours instead. Topics carry their own `dailyCap` on top of the tenant-wide one: three workout reminders a day at most, while everything else flows freely.
