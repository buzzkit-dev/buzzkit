...
- **Steps** carry a `name` (lowercase letters, digits and dashes, unique in the version) except `exit`. Three waits: `wait` says how long, `waitUntil` which moment, `waitFor` which event.
  ...
  - `send` is a message payload (`title`, `body`, `subtitle`, `data`, `topic`, `channel` (`push`), `imageUrl`, `sound`, `badge`, `threadId`, `collapseId`, `interruptionLevel`, `relevanceScore`, `priority`, `deepLink`, `action` (`{ name, data }`, a handler the app registered), `actions` (up to four notification buttons), `policy: "ignore"` (skips the tenant send policy), `deliver` (`push`, or `local`: the message rides to the device early as a silent push and fires as a local notification at the target time, offline included; when the send follows a `waitUntil`, that step's moment is the fire time, canceling the run sends a cancel push, and the SDK acknowledges the plan with `$local.scheduled` — a run that wakes at the fire time without the acknowledgment sends the message as a normal push instead, so no device ever means no notification), `skipIfSentWithin`); at least a title, body or data. `skipIfSentWithin` (a duration) skips the step when the subscriber already received a message with the same `topic` (or from this step when there is none) inside the window.
...
- `repeat: { steps, every, max, until? }` runs its steps, waits `every`, and runs them again — until `until` (any expression; `since: "iteration"` scopes history conditions to the current pass) is met when the loop would repeat, or `max` (2 to 30) passes. `forEach: { items, as, max, steps }` is the data loop: `items` is a scope path to a list, each item is readable as `vars.<as>`, at most `max` (up to 50) items run, in order. Loops do not nest with themselves; a repeat inside a forEach is fine.
- `waitFor` waits on one `event` or several (`events: [{ event, where? }]`, the first match wins — `steps.<name>.event` and `.data` say which). `endOn: [{ event, where? }]` ends the wait unmatched and the run continues (distinct from `cancelOn`, which cancels the run). `resetOn` entries take a `where` too. `settleFor` works with the single-event form.
- `subscriber.channels.<channel>` (an active, enabled subscription exists) and `subscriber.topics.<slug>` (the resolved opt-in) join `subscriber.attributes.*` in expressions and templates, loaded when the spec reads them.
