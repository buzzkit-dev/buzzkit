...
- `PUT /v1/subscribers/:externalId` — identify/upsert: `{ attributes?, email?, subscribeEmail?, timezone? }` → 201 on create, 200 after. `attributes` is free-form JSONB (object-typed, enforced in the DB) (tag data; segments filter on it in engine phase E3), replaced wholesale when present, capped at 64KB serialized. `email` is part of the subscriber: it is written to `attributes.email` (merged into the given `attributes`, so a PUT with both never loses it) and, when the tenant has an email credential, also upserts an email subscription for that address. Without an email credential the address is kept as profile data and no subscription is registered, never a `channel_not_connected`. `subscribeEmail: false` keeps the address as profile data only, even on a tenant with email connected, for addresses that are known but not to be reached. `timezone` (an IANA name, else 400 `invalid_timezone`) sets `$timezone` from your backend, for subscribers whose devices never call the client API, so [scheduled messages](messages.md#scheduled-messages) at each subscriber's local time reach them too; a later client request refreshes it like any system attribute. A create writes `$subscriber.created` to the stream, a change `$subscriber.updated` (both carry the attributes snapshot); an identical PUT writes nothing anywhere.
