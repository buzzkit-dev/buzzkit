...
- `PUT /v1/subscribers/:externalId` — identify/upsert: `{ attributes?, email?, subscribe?, timezone? }` → 201 on create, 200 after. `attributes` is free-form JSONB (object-typed, enforced in the DB) (tag data; segments filter on it in engine phase E3), replaced wholesale when present, capped at 64KB serialized. The subscriber's email is the `email` attribute, and the top-level `email` field is the same thing spelled as a field: it is written to `attributes.email` (merged into the given `attributes`, so a PUT with both never loses it), and either spelling, when the tenant has an email credential, also upserts an email subscription for that address. Without an email credential the address is kept as profile data and no subscription is registered, never a `channel_not_connected`. `attributes.email` that is not an address is a 400 `invalid_email` (`param: attributes.email`). `subscribe` holds the identify options per channel: `subscribe: { email: false }` keeps the address as profile data only, even on a tenant with email connected, for addresses that are known but not to be reached; omitted, every connected channel the profile has an address for is subscribed (`resolveProfileEmail`, `registerProfileEmail`, `upsertSubscriberProfile` in `api/subscribers/registration.ts`). `timezone` (an IANA name, else 400 `invalid_timezone`) sets `$timezone` from your backend, for subscribers whose devices never call the client API, so [scheduled messages](messages.md#scheduled-messages) at each subscriber's local time reach them too; a later client request refreshes it like any system attribute. A create writes `$subscriber.created` to the stream, a change `$subscriber.updated` (both carry the attributes snapshot); an identical PUT writes nothing anywhere.
