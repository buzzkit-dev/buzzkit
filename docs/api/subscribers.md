...
- `PUT /v1/subscribers/:externalId` — identify/upsert: `{ attributes?, email? }` → 201 on create, 200 after. `attributes` is free-form JSONB (object-typed, enforced in the DB) (tag data; segments filter on it in engine phase E3), replaced wholesale when present, capped at 64KB serialized. `email` is sugar that upserts an email subscription. A create writes `$subscriber.created` to the stream, a change `$subscriber.updated` (both carry the attributes snapshot); an identical PUT writes nothing anywhere.
