...
src/
  ...
  pages/                        index, why-buzzkit, pricing, developers, about, contact, privacy, 404, features/[slug] + [slug].md, compare/[slug] +
                                ...
                                developers/llms.txt, og/[...path].png, logo.png, icon.png, apple-touch-icon.png; every endpoint is a renderer
...
- **The hero screens are the dashboard's own pages.** `@buzzkit/web` exports `./components/*`, and the marketing Vite config aliases `@/app/` to `apps/web/app/` so those files resolve; the hero screens import the real WorkflowFlow, SegmentBuilder, badges and DetailRow and mirror the dashboard routes' markup (back button, header, cards, tabs) with sample data. Only pieces that need react-router (subscriber rows, links) are re-drawn locally. Sample workflow specs must pass `lintWorkflow`; step names are slugs. Flags are generated: `bun run flags` mirrors the dashboard's full set (`apps/web/public/flags`, the single source) into the git-ignored `public/flags/`, and `dev` and `build` run it first. Marketing claims `buzzkit.dev/flags/*`, so this worker serves every flag the dashboard renders, not just the ones the hero uses.
...
- **OG images are rendered at build, one per page, and they obey the design system.** Every card is the same hero — wordmark and title centered, the page's visual on a stage below that runs off the bottom edge — and every visual on it is a redraw of a real component with the same radii, tokens, shadows, badges and switches (1.4× on the gradient stage, natural size for the Overview frame); nothing is invented for the card. A page with no product visual falls back to the Overview frame the homepage shows. `lib/og/index.ts` holds the card list (title, gray continuation, a visual per page) and draws it with satori on the subset Open Runde TTFs, titles balanced with `text-wrap: balance` semantics by measuring words with the same font; `Site.astro` takes `image` and every page passes its own `/og/<path>.png`. `logo.png` and `apple-touch-icon.png` come from `favicon.svg` the same way; `icon.png` (1024) comes from `public/icon.svg`, the same logo with its corners rounded, which is the one to link from READMEs and anywhere else the mark is shown outside the site (`buzzkit.dev/icon.png`, `buzzkit.dev/icon.svg`). Never commit a screenshot.
...
- **Design system only.** Tokens, Open Runde for everything except code blocks (mono), the press pattern, `text-balance` titles and `text-pretty` descriptions, sentence case everywhere except CTAs ("Get Started", "Star on GitHub", "Read the API Overview"), the `<title>` of every page and the home hero headline, which are Title Case ("BuzzKit · The Open Source Notification Orchestration Layer", "Page Not Found · BuzzKit"), no em or en dashes in user-facing copy, `font-medium` headings (Natural's weight, never semibold below the hero). One brand accent (`brand-*`, derived from the logo, defined once in `@buzzkit/ui/globals.css` and shared with the dashboard's auth pages); every other color means a status.
...
- **The account CTA is decided before first paint.** The worker stamps `data-signed-in` on `<html>` when the shared session cookie is present, so the header renders both links (`AccountCta`) and the `signed-out-only` / `signed-in-only` utilities in `global.css` hide the wrong one in CSS. Never read the attribute from an effect: that flashes the wrong label. The Astro dev server has no worker, so it always shows Get Started.
...
- **Copy goes through the `copy` skill.** One job per string, full sentences, the API's nouns, honest claims only: no invented limits, pricing or competitor facts. A workspace is one app you send for; multiple apps are multiple workspaces. A tenant is a customer of someone building on BuzzKit. Tenants are not apps, not projects, and not environments. A single-app workspace never mentions tenants.
