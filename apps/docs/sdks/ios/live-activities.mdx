...
curl https://api.buzzkit.dev/v1/live-activities/send \
  ...
  -d '{
    ...
    "alert": { "title": "Kick-off", "body": "The match has started." }
  }'
```
...
| `alert` | `{ title, body, sound }`, shown as a banner alongside the update. Required for `start`: iOS discards a push-to-start without one, so the API answers `alert_missing`. |
