...
import { FilterBar, FilterSearch } from '@buzzkit/ui/components/filter-bar';
...
import { ApiError, listEveryWorkspace } from '@/app/lib/api.server';
...
export default function AdminRoute({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className='flex min-h-svh flex-col gap-2 bg-background-subtle p-2'>
      ...
      <div className='corner-superellipse/1.125 flex min-w-0 flex-1 flex-col overflow-y-auto rounded-2xl bg-card px-4 pt-5 shadow-sm sm:px-6 sm:pt-6 lg:px-8.5 lg:pt-7.5'>
        <div className='flex min-h-0 w-full flex-1 flex-col gap-5 pb-5 lg:pb-7.5'>
          <PageHeader
            title='Admin'
            description='Every workspace on this deployment. You open any of them as an owner, and what you change is recorded in its audit log as BuzzKit Support.'
          />
          <FilterBar>
            <FilterSearch
