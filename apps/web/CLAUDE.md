...
app/
  ...
  components/<feature>/         Only pieces used by more than one route live here, one directory per feature, files named for what they
                                ...
                                icon and detail per `$` name, custom names as themselves; name.tsx, the one-line glyph + label cell of the catalog and the stream; volume-chart.tsx, shared by the Events pages) · webhooks/ (describe.ts, the subscription summary shared by the list and the endpoint page) · auth/ (form, providers, password input) · layout/ (the signed-in chrome: sidebar + navigation.ts, the IA;
