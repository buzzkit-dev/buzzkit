...
describe('serializeDelivery', () => {
  ...
  it('exposes the delivery with prefixed ids', () => {
    expect(serializeDelivery(delivery)).toEqual({
  ...
  it('carries the joined event type when the query provides one', () => {
    expect(serializeDelivery({ ...delivery, eventType: 'tenant.created' }).eventType).toBe('tenant.created');
    expect(serializeDelivery(delivery).eventType).toBeNull();
  });
...
describe('serializeAttempt', () => {
  ...
  it('exposes the attempt with a prefixed id and without the delivery id', () => {
    expect(serializeAttempt(attempt)).toEqual({
    ...
    expect(serializeAttempt(attempt).id).toMatch(/^wha_/);
    expect(serializeAttempt(attempt)).not.toHaveProperty('deliveryId');
  });

  it('passes a network failure through', () => {
    expect(
      serializeAttempt({ ...attempt, status: null, error: 'timeout', responseBody: null })
    ).toMatchObject({
      status: null,
      error: 'timeout',
      responseBody: null,
