...
export const topic = new Elysia()
  ...
  .get(
    ...
    async ({ db, params, tenant }) => {
      const target = await findTopicBySlug(db, tenant.id, params.topicSlug);
      return Response.success(serializeTopic(target), { entity: 'topic' }).send();
    },
    { tenant: 'topics:read' }
  )
  .patch(
    ...
    async ({ body, db, params, tenant, audit }) => {
      const target = await findTopicBySlug(db, tenant.id, params.topicSlug);
      const channels = body.channels ?? target.channels;
      ...
      const updated = await updateTopic(db, tenant.id, target.id, {
        ...body,
        channelDefaults: resolveChannelDefaults(body.channelDefaults ?? target.channelDefaults, channels),
      });

      await audit({
        event: 'topic.updated',
        tenantId: tenant.id,
        target: { type: 'topic', id: target.id },
  ...
  .delete(
    ...
    async ({ db, params, tenant, audit }) => {
      ...
      const deleted = await softDeleteTopic(db, target.id);

      await audit({
        event: 'topic.deleted',
        tenantId: tenant.id,
        target: { type: 'topic', id: target.id },
        data: { slug: target.slug, name: target.name },
      });
