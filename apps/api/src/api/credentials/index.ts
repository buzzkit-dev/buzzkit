...
import { and, channel, type Db, desc, environment, eq, isNull, lt, tables } from '@buzzkit/database';
...
export async function listConnectedChannels(db: Db, tenantId: number): Promise<ConnectedChannel[]> {
  ...
  return channel.enumValues.filter((entry) => connected.has(entry));
}
