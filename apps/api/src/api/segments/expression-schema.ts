...
export const DurationSchema = Type.String({ pattern: DURATION_PATTERN.source });

export const EventNameSchema = Type.String({ pattern: EVENT_NAME_PATTERN.source });
...
export function countConditionSchema(window: Record<string, TSchema>) {
  return Type.Object(
    {
      count: EventNameSchema,
...
export const CountConditionSchema = countConditionSchema({});

export const NeverConditionSchema = Type.Object(
  { never: EventNameSchema, within: Type.Optional(DurationSchema) },
  { additionalProperties: false }
);
...
export const ExpressionSchema: TSchema = expressionSchema(ConditionSchema, 'Expression');
