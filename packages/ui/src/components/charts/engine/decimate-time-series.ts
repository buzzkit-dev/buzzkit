export function decimateTimeSeries<T extends Record<string, unknown>>(
  ...
  const yOf = (point: T, index: number): number => {
  ...
  for (let i = 0; i < maxPoints - 2; i++) {
    ...
    let avgX = len - 1;
    let avgY = yOf(data[len - 1] as T, len - 1);
    if (nextCount > 0) {
      avgX = 0;
      avgY = 0;
      for (let j = nextRangeStart; j < nextRangeEnd; j++) {
        avgX += j;
        avgY += yOf(data[j] as T, j);
      }
      avgX /= nextCount;
      avgY /= nextCount;
    }
    ...
    const ay = yOf(pointA, previousIndex);

    let maxArea = -1;
    let maxIndex = rangeStart;

    for (let j = rangeStart; j < rangeEnd; j++) {
      const area = Math.abs((ax - avgX) * (yOf(data[j] as T, j) - ay) - (ax - j) * (avgY - ay)) * 0.5;
      if (area > maxArea) {
        maxArea = area;
        maxIndex = j;
      }
