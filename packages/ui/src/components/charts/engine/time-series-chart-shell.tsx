...
} from '../loading/generate-chart-skeleton-data';
import {
  extractProjectionLineConfigs,
  mergeProjectionXDomainMax,
  mergeProjectionYDomain,
} from '../overlays/projection-config';
import { extractReferenceAreaConfigs, type ReferenceAreaConfig } from '../overlays/reference-area-config';
import { ReferenceAreaRegistrationContext } from '../overlays/reference-area-registration-context';
...
export interface TimeSeriesChartInnerProps {
  ...
  /** Whether hover shows the crosshair, tooltip and cursor. Default: true */
  interactive?: boolean;
...
const TimeSeriesChartCore = memo(function TimeSeriesChartCore({
  ...
  const canInteract = interactive && isLoaded && isChartInteractionPhase(chartPhase);
