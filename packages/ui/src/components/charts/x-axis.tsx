...
import { useChart, useChartStable } from './engine/chart-context';
import { shortDateFmt } from './engine/chart-formatters';
import { DEFAULT_Y_DOMAIN_TWEEN_MS } from './engine/chart-phase';
...
export interface XAxisProps {
  ...
  /** Distance from the chart's bottom edge to the labels, in px. */
  offset?: number;
  /** Pick which data points carry a label; overrides `numTicks` in data mode. */
  ticks?: (date: Date, index: number) => boolean;
  /** Format a label from its date. Default: the short date. */
...
function XAxisLabel({
  ...
  let opacity = 1;
  if (isHovering && crosshairX !== null && tickerHalfWidth > 0) {
...
function buildDataAlignedTicks({
  ...
  pick?: (date: Date, index: number) => boolean;
  format?: (date: Date, index: number) => string;
}): AxisTick[] {
  const seenLabels = new Set<string>();
  ...
  for (const index of indices) {
    ...
    const label = format ? format(date, index) : (dateLabels[index] ?? shortDateFmt.format(date));
    if (seenLabels.has(label)) {
      continue;
    }
    seenLabels.add(label);
...
const XAxisInner = memo(function XAxisInnerBase({
  ...
  const labelsToShow = useMemo(() => {
    ...
    return dataTicks;
  }, [tickMode, xDomain, data, dateLabels, xAccessor, xScale, margin.left, numTicks, ticks, format]);
  ...
  const hoveredLabel =
    isHovering && tooltipData
      ? format
        ? format(xAccessor(tooltipData.point), tooltipData.index)
