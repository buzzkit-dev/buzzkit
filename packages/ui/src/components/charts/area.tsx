...
import { chartCssVars, useChartStable, useYScale } from './engine/chart-context';
import type { ChartPhase, LoadingStyle } from './engine/chart-phase';
import {
  type LineLoadingPulseMode,
  LineLoadingPulseStroke,
  resolveLineLoadingPulseMode,
} from './loading/line-loading-pulse';
import { LINE_LOADING_LOOP_PAUSE_MS } from './loading/line-loading-timing';
import { LineLoadingSweep } from './loading/loading-sweep';
import { AreaGradientDefs } from './series/area-gradient-defs';
import { type FadeEdges, resolveFadeSides } from './series/fade-edges';
import { resolveDashTailBounds, usePathStrokeMetrics } from './series/path-stroke-utils';
...
import { seriesAreaFromPoints } from './series/series-path-utils';
import type { SeriesPointMarkerStyle } from './series/series-point-marker';
import { useAnimatedSeriesPath } from './series/use-animated-series-path';
...
export function Area({
  ...
  const { points, pathD: lineD } = useAnimatedSeriesPath({
    renderData,
    xAccessor,
    xScale,
    yScale,
  ...
  const baselineY = useMemo(() => {
    const scaleWithRange = yScale as { range?: () => number[] };
    const range = scaleWithRange.range?.();
    return range && range.length > 0 ? Math.max(...range) : innerHeight;
  }, [yScale, innerHeight]);
  const areaD = useMemo(() => seriesAreaFromPoints(points, curve, baselineY), [points, curve, baselineY]);
  ...
  const seriesLayers = (
    <>
      {showSeriesContent && showAreaFill ? <path d={areaD} fill={areaFill} /> : null}
