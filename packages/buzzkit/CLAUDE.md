...
The public `buzzkit` package: what customers install, and what the platform (`apps/api`) dogfoods. One package, organized by subpath exports (`buzzkit/webhooks` and `buzzkit/expressions` today; channels, workflows, campaigns, the send client and device token APIs as they land), never split into separate npm packages for organization's sake.
